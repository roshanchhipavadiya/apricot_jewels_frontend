...
import { useGetSubCategoriesQuery, useGetCategoriesQuery, useGetProductQuery, useAddProductMutation } from "../services/apiSlice";
...
const daily_logs = () => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (!form.gst?.trim()) return toast.error("GST is required");
            if (!form.making_charges?.trim()) return toast.error(" Making Charges is required");
            // if (!form.stock || isNaN(form.stock)) return toast.error("Valid stock is required");
            // if (!form.sku?.trim()) return toast.error("SKU is required");
            if (!form.product_weight_g || isNaN(form.product_weight_g)) return toast.error("Valid product weight is required");

            // === Prepare image ===
            const fileData =
                form.product_img instanceof File
                    ? form.product_img
                    : "";

            if (!fileData) return toast.error("Product image is invalid");

            // === Prepare FormData ===
            const formdata = new FormData();
            formdata.append('product_id', localStorage.getItem('product_id'));
            formdata.append('product_img', fileData);
            ...
            formdata.append('height_mm', form.height_mm);
            formdata.append('width_mm', form.width_mm);
            formdata.append('length_inches', form.length_inches);
            formdata.append('product_weight_g', form.product_weight_g);
            formdata.append('gender', form.gender);
            formdata.append('is_ring_size', form.is_ring_size);
            formdata.append('trending', capitalizeBoolean(form.trending));
            formdata.append('bestseller', capitalizeBoolean(form.bestseller));
            formdata.append('authenticity', capitalizeBoolean(form.authenticity));

            // === Submit ===
            const response = await addProduct(formdata).unwrap();
            toast.success("Product added successfully");
            setTimeout(() => {
                navigate('/product', { replace: true }); // Replace this with your actual product page path
            }, 1500);
    ...
    const handleDropdownSelect = (field, value) => {
        // For gender, it can stay as is ("Male"/"Female"), but for others like "trending" it will need "True"/"False"
        const newValue =
            field === "gender" ? value :
                value === "Active" ? "True" :
                    value === "Inactive" ? "False" : value;
        ...
        <div>
            ...
            <div className='flex inter'>
                ...
                <div className="w-full width__right relative max-md:ms-0">
                    ...
                    <div className="flex justify-between gap-[10px] mb-[27px] flex-wrap items-center max-sm:block"  >
                        ...
                        <nav className="text-sm text-gray-600 max-sm:mt-[15px] max-sm:ps-[5px]" aria-label="Breadcrumb" >
                            <ol className="flex flex-wrap items-center ">
                                ...
                                <li className="text-primary font-medium text-[12px]">Add Product Details</li>
                            </ol>
                        </nav>
                    ...
                    <div>
                        <div className=" mt-[30px]  p-[30px] max-lg:p-[0px]   rounded-[10px]">
                                ...
                                <div className="flex flex-col xl:flex-row gap-6">
                                    ...
                                    {/* <div className="mb-4">
                                        ...
                                        <div className="mt-[10px] w-full flex flex-col items-center">
                                            ...
                                            <div className="relative w-full flex items-center h-[40px] px-[15px] border border-[#C8C8C8] text-[#9CA3AF] rounded-[7px] bg-transparent text-sm cursor-pointer">
                                                <input
                                                    type="file"
                                                    accept="image/*"
                                            ...
                                            <div className="w-full mt-4">
                                                {selectedImage ? (
                                                    <div className="w-full rounded-[10px] overflow-hidden bg-white flex items-center justify-center">
                                                        <img
                                                            src={URL.createObjectURL(selectedImage)}
                                                            alt="Selected"
                                                            className="h-full w-full min-h-[100px] object-contain"
                                                        />
                                                    </div>
                                                ) : (
                                                    <div
                                                        onClick={handleUploadClick}
                                                        className="h-[200px] border-2 border-dashed border-gray-300 rounded-lg bg-gray-50 flex items-center justify-center cursor-pointer"
                                                    >
                                    ...
                                    <div className="flex flex-col w-[30%] max-xl:w-[50%] items-center max-lg:w-[100%]   h-auto ">
                                        {/* Hidden File Input */}
                                        <input
                                            type="file"
                                            accept="image/*"
                                        ...
                                        <div
                                            className={`w-full overflow-hidden bg-white flex items-center justify-center rounded-[10px] cursor-pointer  ${selectedImage || (form.product_img && typeof form.product_img === "string") ? 'border-0' : ''
                                                }`}
                                            onClick={handleUploadClick}
                                        >
                                            {selectedImage || (form.product_img && typeof form.product_img === "string") ? (
                                                ...
                                                <div className="w-full h-auto  min-h-[350px] max-lg:min-h-[250px] max-lg:w-[250px] bg-gray-200 flex items-center justify-center border-2 border-dashed rounded-lg">
                                    ...
                                    {/* <div className="w-[70%]  bg-white rounded-[10px] p-4" >
                                        ...
                                        <div className="mb-[60px]">
                                            ...
                                            <div className="grid grid-cols-2 gap-4 ">
                                                ...
                                                {/* <Searchdropdown
                                                    ...
                                                    setIsOpen={(state) => setOpenDropdown(state ? "category" : null)}
                                                    onSelect={(value) => {
                                                ...
                                                <Searchdropdown
                                                    ...
                                                    onSelect={(value) => {
                                                        ...
                                                        setform({ ...form, category: selectedCategory?.id }); // Reset subcategory
                                                    }}
                                                />
                                                ...
                                                {/* <Searchdropdown
                                                    ...
                                                    setIsOpen={(state) => setOpenDropdown(state ? "subcategory" : null)}
                                                    onSelect={(value) => {
                                                ...
                                                <Searchdropdown
                                                    ...
                                                    setIsOpen={(state) => setOpenDropdown(state ? "subcategory" : null)}
                                                    onSelect={(value) => {
                                                        const selectedSubcategory = subcategoryopation.find((sub) => sub.name === value);
                                                        setform({ ...form, subcategory: selectedSubcategory?.id });
                                                    }}
                                                />
                                            ...
                                            <div className="">
                                                ...
                                                <div className="grid grid-cols-4 gap-4">
                                                    <Dropdown
                                                        ...
                                                        value={form.trending === "True" ? "Active" : "Inactive"} // Display "Active" or "Inactive" based on the form state
                                                        onSelect={(val) => handleDropdownSelect("trending", val)} // Handle value change for trending
                                                        isOpen={openDropdown === 1} // Checking if this dropdown is open
                                                        onToggle={() => toggleDropdown(1)} // Toggle open/close of the dropdown
                                                        ref={(el) => (dropdownRefs.current[1] = el)} // Ref for this dropdown
                                                    />
                                                    {/* Bestseller Dropdown */}
                                                    <Dropdown
                                                        label="Bestseller"
                                                        options={["Active", "Inactive"]}
                                                        value={form.bestseller === "True" ? "Active" : "Inactive"} // Display "Active" or "Inactive" based on the form state
                                                        onSelect={(val) => handleDropdownSelect("bestseller", val)} // Handle value change for bestseller
                                                        isOpen={openDropdown === 2} // Checking if this dropdown is open
                                                        onToggle={() => toggleDropdown(2)} // Toggle open/close of the dropdown
                                                        ref={(el) => (dropdownRefs.current[2] = el)} // Ref for this dropdown
                                                    />
                                                    ...
                                                    <Dropdown
                                                        label="Authenticity"
                                                        options={["Active", "Inactive"]}
                                                        value={form.authenticity === "True" ? "Active" : "Inactive"} // Display "Active" or "Inactive" based on the form state
                                                        onSelect={(val) => handleDropdownSelect("authenticity", val)} // Handle value change for authenticity
                                                        isOpen={openDropdown === 3} // Checking if this dropdown is open
                                                        onToggle={() => toggleDropdown(3)} // Toggle open/close of the dropdown
                                                        ref={(el) => (dropdownRefs.current[3] = el)} // Ref for this dropdown
                                                    />
                                                    <Dropdown
                                                        label="Ring Size"
                                                        options={["Active", "Inactive"]}
                                                        value={form.is_ring_size === "True" ? "Active" : "Inactive"} // Display "Active" or "Inactive" based on the form state
                                                        onSelect={(val) => handleDropdownSelect("is_ring_size", val)} // Handle value change for authenticity
                                                        isOpen={openDropdown === 4} // Checking if this dropdown is open
                                                        onToggle={() => toggleDropdown(4)} // Toggle open/close of the dropdown
                                                        ref={(el) => (dropdownRefs.current[4] = el)} // Ref for this dropdown
                                                    />
                                                </div>
                                            </div>
                                    ...
                                    {/* <div className=" grid grid-cols-2  gap-5 ">
                                                <Searchdropdown
                                                    ...
                                                    onSelect={(value) => {
                                                        ...
                                                        setform({ ...form, category: selectedRoom?.category_id });
                                                    }}
                                                />
                                                <Searchdropdown
                                                    label="Sub Category"
                                                    options={subcategoryopation?.map((val) => val.name)} // Display only names
                                                    main_color="text-gray"

                                                    labelclassName="text-[#9CA3AF]"
                                                    onSelect={(value) => {
                                                        const selectedRoom = subcate?.data.find(category => category.name === value);
                                                        console.log(selectedRoom)
                                                        setform({ ...form, subcategory: selectedRoom?.subcategory_id });
                                                    }}
                                                />
                                                <div>
                                                ...
                                                <div>
                                                    ...
                                                    <div className="relative">
                                                        ...
                                                        <input type="date" onChange={(e) => setform({ ...form, manufacture_date: e.target.value })} onFocus={(e) => e.target.showPicker()} placeholder="dd/mm/yyyy" className="mt-[10px] w-[100%]  h-[35px] sm:h-[40px] dateInput sm:px-[15px] px-[12px] border border-[#C8C8C8] text-gray rounded-[7px] bg-transparent  text-sm focus:ring-blue-500 focus:border-blue-500" />
                        ...
                        <div className="flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0">
                            <div className="relative transform overflow-hidden   p-[30px] rounded-[30px] bg-white text-left shadow-xl transition-all sm:my-8  w-full sm:max-w-2xl">
                                ...
                                <div className=" pt-[30px] flex flex-row-reverse  gap-3">
                                    <button type="button" onClick={handleAddCategory} className="inline-flex  items-center justify-center h-[35px] sm:h-[40px]  shadow-[0px_8px_20px_1px_#3DB0F733] rounded-md bg-[#D86A37]    w-[154px] py-2 text-sm font-semibold text-white shadow-xs  sm:ml-3 ">Add Category</button>
                                    <button type="button" onClick={() => closeModal()} className=" items-center closeModal h-[35px] sm:h-[40px]  inline-flex  justify-center rounded-md bg-white    py-2 w-[114px] text-sm font-semibold text-gray-900 ring-1 shadow-xs ring-gray-300 ring-inset hover:bg-gray-50 sm:mt-0 ">Cancel</button>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
                            <div className="flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0">
                                <div className="relative transform overflow-hidden   p-[30px] rounded-[30px] bg-white text-left shadow-xl transition-all sm:my-8  w-full sm:max-w-2xl">
                                    ...
                                    <div className=" pt-[30px] flex flex-row-reverse  gap-3">
                                        <button type="button" onClick={handleEditCategory} className="inline-flex  items-center justify-center h-[35px] sm:h-[40px]  shadow-[0px_8px_20px_1px_#3DB0F733] rounded-md bg-[#D86A37]    w-[154px] py-2 text-sm font-semibold text-white shadow-xs  sm:ml-3 ">Edit Category</button>
                                        <button type="button" onClick={() => closeModal()} className=" text-[#D86A37] items-center closeModal h-[35px] sm:h-[40px]  inline-flex  justify-center rounded-md bg-white    py-2 w-[114px] text-sm font-semibold text-gray-900 ring-1 shadow-xs ring-gray-300 ring-inset hover:bg-gray-50 sm:mt-0 ">Cancel</button>
                                    </div>
                                </div>
                            </div>
                        </div>
