...
import { useGetDiamondQuery, useDeleteDiamondMutation, useGetProducts11Query } from "../services/apiSlice";
...
const Daimond = () => {
    ...
    // Pass it to RTK Query
    const { data: diamondData, error, isLoading } = useGetDiamondQuery({ product_id: productId });
    ...
    const filteredDiamonds = data.filter(
        (diamond) => diamond.vpo == null
    );
    console.log(data);
    const totalPages = Math.ceil((filteredDiamonds?.length || 0) / itemsPerPage);


    const filteredData = filteredDiamonds?.filter(item =>
        item.diamond_shape?.toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    return (
        <div>
            ...
            <div className='flex inter'>
                ...
                />
                <div className="w-full width__right relative max-md:ms-0" onClick={() => setActiveDropdown(null)}>
                    ...
                    <div className="flex justify-between gap-[10px] mb-[27px] flex-wrap items-center max-sm:block" >
                        ...
                        <nav className="text-sm text-gray-600 max-sm:mt-[15px] max-sm:ps-[5px]" aria-label="Breadcrumb" >
                            <ol className="flex flex-wrap items-center ">
                                ...
                                <li className="text-primary font-medium text-[12px]">Daimond Details</li>
                            </ol>
                        </nav>
                    ...
                    <div className="border mt-[30px] bg-white border-[#EBE9F1] shadow-[0px_4px_24px_0px_#0000000F]  rounded-[10px]">
                        ...
                        <div className="min-h-[65vh] over_scroll">
                            ...
                            <table className="w-full ">
                                ...
                                <tbody className="last_tr">
                                    {displayedData?.length > 0  ? (
                                        displayedData.map((diamond, index) => (
                                            <tr
                                                key={index}
                                                ...
                                                onClick={() => navigate("/edit_diamond", { state: { diamondId: diamond?.diamond_id, component: diamond?.component } })}
                                            >
                                                ...
                                                >
                                                    ...
                                                    <div className="flex gap-2 max-lg:gap-0 justify-center">
                                                        <Link
                                                            to="/edit_diamond"
                                                            state={{ diamondId: diamond?.diamond_id, component: diamond?.component }}
