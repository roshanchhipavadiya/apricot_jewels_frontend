...
import { useGetMetalQuery, useDeleteMetalMutation, useGetProducts11Query } from "../services/apiSlice";
...
const Metal = () => {
    ...
    const { data: MetalData, error, isLoading } = useGetMetalQuery({ product_id: productId });

    const [deleteMetal] = useDeleteMetalMutation();
    const [deleteMetalId, setDeleteMetalId] = useState(null);

    const data = MetalData?.data || [];
    console.log(data);
    ...
    const handleDeleteMetal = async () => {
        try {
            await deleteMetal(deleteMetalId).unwrap();
    ...
    const filteredData = data?.filter(item =>
        ...
        item.metal_color?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.metal_purity?.toLowerCase().includes(searchTerm.toLowerCase())
    );
                        ...
                        <div className="flex justify-between gap-[10px] mb-[27px] flex-wrap items-center max-sm:block" >
                            ...
                            <nav className="text-sm text-gray-600 max-sm:mt-[15px] max-sm:ps-[5px]" aria-label="Breadcrumb" >
                                <ol className="flex flex-wrap items-center ">
                                    ...
                                    <li className="text-primary font-medium text-[12px]">All Metal</li>
                                </ol>
                            </nav>
                                ...
                                <table className="w-full ">
                                    <thead className="">
                                        <tr className="table-flex items-center h-[40px] sm:h-[54px] ">
                                            {/* <th className="text-start px-[30px] text-xs font-semibold text-[#5E5873] uppercase">Image</th> */}
                                            <th className=" text-start px-[30px] text-[14px] font-[700]   max-md:px-[20px] max-md:text-[13px]"> Color</th>
                                            <th className=" text-[14px] font-[700] text-center max-md:px-[1px] max-md:text-[13px] max-sm:px-[5px]"> Price</th>
                                            <th className=" text-[14px] font-[700] text-center max-md:px-[1px] max-md:text-[13px] max-sm:px-[5px] "> Purity </th>
                                            <th className=" text-[14px] font-[700] text-center max-md:px-[1px] max-md:text-[13px] max-sm:px-[5px] max-lg:hidden"> Type </th>
                                            <th className=" text-[14px] font-[700] text-center max-md:px-[1px] max-md:text-[13px] max-sm:px-[5px] max-sm:hidden"> Weight(gm)</th>
                                        ...
                                        {displayedData?.length > 0 ? (
                                            displayedData.map((metal, index) => (
                                                <tr
                                                    ...
                                                    onClick={() => navigate("/edit_metal", { state: { metalId: metal?.metal_id } })}


                                                >

                                                    <td className=" text-sm text-[#5E5873] max-md:text-[12px] px-[30px]  max-md:px-[20px]   max-md:max-w-[100px] max-sm:text-[11px]">{metal?.metal_color}</td>

                                                    <td className="text-sm text-[#5E5873] text-center px-[30px] max-sm:text-[11px]">{metal?.metal_price}</td>
                                                    <td className="text-sm text-[#5E5873] text-center max-sm:text-[11px]">{metal?.metal_purity}</td>
                                                    <td className="text-sm text-[#5E5873] text-center max-lg:hidden">{metal?.metal_type}</td>

                                                    <td className="text-sm text-[#5E5873] text-center  max-sm:hidden">{metal?.metal_weight_g}</td>
                                                            ...
                                                            <ul className={`absolute top-[-10px] right-2 shadow-lg bg-white rounded-[6px] p-[5px] border border-[#EBE9F1] mt-2 z-[10]  ${openDropdown === index ? 'block' : 'hidden'}`}>
                                                                ...
                                                                <li>
                                                                    <button className="flex items-center justify-between w-[112px] p-[5px] text-sm bg-white transition-all hover:bg-[#FFD6D6] rounded-[4px]" onClick={() => {
                                                                        openModal("modal3");
                                                                        setDeleteMetalId(metal?.metal_id); 
                                                                    }}>
                                ...
                                <div className="flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0">
                                    <div className="relative transform overflow-hidden p-[30px] border-t-[8px] border-[#F44336] rounded-[6px] bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-xl z-40">
                                        ...
                                        <div className="  pt-[30px] flex flex-row-reverse gap-3 ">
                                            <button type="button" onClick={handleDeleteMetal} className="inline-flex h-[35px] sm:h-[40px]  w-[114px] shadow-[0px_8px_20px_1px_#F443364D] rounded-md bg-[#F44336] items-center   justify-center py-2 text-sm font-semibold text-white shadow-xs  sm:ml-3 ">Delete</button>
